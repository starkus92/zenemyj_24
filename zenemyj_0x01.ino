...
/*!
     ...
     @section HISTORY 
     v2.3 -Exportar Data hacia Excel
...
//Declaramos puertos seriales
 #define orprx 2 // Definimos pin ORP RX, recepcion
 #define orptx 3 // Definimos pin ORP TX, transmision
 SoftwareSerial orpserial(orprx, orptx); // Declaramos el puerto serial ORP
 ...
 // Declaramos las variables del ORP
 char orp_data[20]; // Arreglo de 20 bytes para guardas los valores del ORP
 char orp_computerdata[20]; // Arreglo de 20 bytes  para guardar los valores desde el PC
 byte orp_received_from_computer=0; // Caracteres recibidos desde el PC
 byte orp_received_from_sensor=0; // Caracteres recibidos del sensor de pH.
 byte orp_startup=0; // Comprobamos la conexion de la placa con el ORP
 float ORP=0; // Asignamos un float al valor del ORP registrado
 byte orp_string_received=0; // Este byte es usado como flag para indicar que hemos recibido el string del modulo ORP
 ...
 orpserial.begin(38400); // Habilitamos el puerto serial del ORP
 ...
 orpserial.listen();
 delay(100);
 if(orpserial.available() > 0){ // Solo si necesitamos que la longitud del datos del ORP sea la de n caracter
 // Leemos los datos del circuito ORP hasta detectar un <CR> al mismo tiempo que contamos cuantos char´s recibimos
 orp_received_from_sensor=orpserial.readBytesUntil(13,orp_data,20); 
 ...
 lcd.setCursor(7,0); // Configuramos la posicion del ORP en el LCD
 lcd.print("ORP:");
 lcd.print(ORP, 0); // Enviamos valores de ORP al LCD
 ...
 orpserial.print("R\r"); //Obtenemos una lectura del ORP.
 if(orp_string_received==1){ // Comprobamos que hayamos tomado un dato
 ORP=atof(orp_data); // Convertimos el string "orp_data" hacia float
 if(ORP>800){Serial.println("high\r");} // Comprobamos que haya sido convertido al string.
 if(ORP<800){Serial.println("low\r");} // Comprobamos que haya sido convertido al string.
 orp_string_received=0;} // Reseteamos el flag del string.
 ...
 File dataFile = SD.open("datalog.txt", FILE_WRITE); // Abrimos el archivo
 ...
 dataFile.println(ORP); // Registramos el ORP
 dataFile.print(","); //Separamos con otra coma mas
